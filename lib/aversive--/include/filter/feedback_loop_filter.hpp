...
private:
  typename Config::ConsignFilter& _cf;
  typename Config::FeedbackFilter& _ff;
  typename Config::ErrorFilter& _ef;

  FeedbackInputType feedback;

public:
  inline FeedbackLoopFilter(decltype(_cf)& cf, decltype(_ff)& ff, decltype(_ef)& ef) : _cf(cf), _ff(ff), _ef(ef), feedback(0) {}
